...
class XDerRPC(CasperModel):
    ...
    def get_name(self):
        return 'XDerRPC' + self.get_name_extension()
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        loss_casper = torch.tensor(0.)
        if self.task > 0 and self.args.rep_minibatch > 0 and self.args.rho > 0:
            loss_casper = self.get_replay_loss()
            loss_casper *= self.args.rho

        loss = loss_stream + loss_der + loss_derpp + loss_constr_futu + loss_constr_past + loss_casper
