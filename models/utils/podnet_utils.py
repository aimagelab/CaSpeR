...
from models.utils import distance as distance_lib
...
def extract_features(model, loader, features_type="features"):
    assert features_type in ["features", "raw_features"], "Unknown features type."
    targets, features = [], []
    ...
    for input_dict in loader:
        ...
        _features = model.forward_all(inputs.to(model.device)).detach().cpu().numpy()
        _features = _features[features_type]
